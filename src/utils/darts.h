/*
  ...
  $Id: darts.h.in 1575 2007-01-27 13:24:27Z taku $;
...
namespace Darts {
      ...
      for (size_t i = 0; i < siblings.size(); ++i) {
        ...
        if (!fetch(siblings[i], new_siblings)) {
          array_[begin + siblings[i].code].base =
            ...
            static_cast<array_type_>(-1*siblings[i].left-1);
  ...
  public:
	...
	void set_result(result_pair_type& x, value_type r, size_t l,array_u_type_ p ) {
		x.value = r;
		x.length = l;
		x.pos = p;
	}
    ...
    int build(size_t     key_size,
      ...
      value_         = value;
      progress_      = 0;
	  //lower down the value to build very large(1450K word) ucs2 darts.
      resize(key_size_*64); //LMN: This line is the reason why word-item can not larger than 255
    ...
    int open(const char *file,
      ...
      if (!fp) return -1;
      if (std::fseek(fp, (long)offset, SEEK_SET) != 0) return -1;
    ...
    size_t commonPrefixSearch(const key_type *key,
                              ...
                              size_t len = 0,
                              size_t node_pos = 0) {
		  return commonPrefixSearch(key, 0xFFFF, result, result_len, len, node_pos);
	}
    ...
    size_t commonPrefixSearch(const key_type *key,
      ...
      if ((array_u_type_)b == array_[p].check && n < 0) {
        ...
        ++num;
		  ...
		  //all prev-string match. but not term. pos  < 0, this is a prefix
		  if (num < result_len) set_result(result[num], -n-1, len, -1*p);
		  ++num;
	  }
	 ...
	 template <class T>
    size_t commonPrefixSearch(const u4 *key,
							  u4 flag,
                              T* result,
                              size_t result_len,
                              size_t len = 0,
...
#endif
  typedef Darts::DoubleArrayImpl<unsigned short, unsigned short, int,
	  unsigned int> DoubleArrayUcs2;
}
#endif
